...
output_directory = "/Users/Kevin/Downloads/"
input_directory = "/Volumes/UMF_SA_2017_01/2017_UMF_SA/02_Footage/"
    ...
    for file in files_to_convert:
        ...
        try:
            ...
            if not check_if_file_exists(file):
                ...
                print ("Width" + str(file_width) +
                       " Height " + str(file_height))

                print ("Proxy width " + str(proxy_width) +
                       "Proxy Height " + str(proxy_height))

                # Encode the proxy
                print("test")
                subprocess.run(["REDline", "--useRMD", "1", "--i", file, "--outDir", output_directory, "--o", file_name + "_Proxy", "--format",
                                "201", "--PRcodec", "2", "--res", "4", "--resizeX", str(proxy_width), "--resizeY", str(proxy_height)], shell=False)
                converted_count += 1
                logging.info(str(converted_count) + " of " +
                             str(len(files_to_convert)) + " have been converted!")
            else:
